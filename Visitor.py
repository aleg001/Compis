...
class yaplVisitor(ParseTreeVisitor):
        ...
        if hasattr(ctx, "formal") and callable(getattr(ctx, "formal")):
            ...
            for declaracion in x:
                ...
                if len(relative_scope) == 1:
                    self.tabla.fila(self.scope[-1], "self").inside.append(
                        declaracion.ID().getText()
                    )
                ...
                if len(relative_scope) > 1:
                    self.tabla.fila(
                        relative_scope[-1], relative_scope[-2]
                    ).inside.append(declaracion.ID().getText())
        ...
        # * Si dentro del metodo, existe un bloque
            ...
            for xxxx2 in expresiones_todas:
                ...
                if isinstance(xxxx2, yaplParser.DispatchImplicitBContext):
                    ...
                    if xxxx2.ID().getText() in metodos_especiales_string:
                        ...
                        for xxxx in mini_expr:
                            if isinstance(xxxx, yaplParser.StringContext):
                                ...
                                print(f"        PUSH t0")
                                print(
                                    f"        CALLING {xxxx2.ID().getText()} POP LAST"
                                )
                            if isinstance(xxxx, yaplParser.DispatchExplicitAContext):
                                ...
                                if (contar_apariciones(xxxx.getText(), ").")) == 2:
                                    ...
                                    for indice, parametrito in enumerate(
                                        primer_nivel[1]["parametros"]
                                    ):
                                        if indice != 0:
                                            print(
                                                f"        t0 = {parametrito.getText()}"
                                            )
                                            print(f"        PUSH t0")
                                    for indice, parametrito in enumerate(
                                        primer_nivel[1]["parametros"]
                    ...
                    if xxxx2.ID().getText() in metodos_especiales_int:
                        mini_expr = xxxx2.expr()
                                ...
                                for indice, parametrito in enumerate(
                                    ...
                                    if indice != 0:
                                        ...
                                        if self.metodo_scope[-1] == " ":
                                            ...
                                            relative_scope.append(self.scope[-1])
                                            relative_scope.append(self.metodo_scope[-1])
                                        if self.tabla.is_in_table(
                                            parametrito.getText(), relative_scope
                                        ):
                                            informacion_fila = self.tabla.fila(
                                                parametrito.getText(), relative_scope
                                            )
                                            externo = f"fp[{informacion_fila.offset}]"
                                            print(f"        t0 = {externo}")
                                            print(f"        PUSH t0")
                                        elif self.tabla.is_in_table(
                                            parametrito.getText(), [self.scope[-1]]
                                        ):
                                            informacion_fila = self.tabla.fila(
                                                parametrito.getText(), [self.scope[-1]]
                                            )
    ...
    def visitPropiedad(self, ctx: yaplParser.PropiedadContext):
        ...
        if hasattr(ctx, "ASSIGNMENT") and callable(getattr(ctx, "ASSIGNMENT")):
            ...
            # * EXPR 16
            if hasattr(ctx.expr(), "STRING") and callable(
                getattr(ctx.expr(), "STRING")
            ):
            ...
            if isinstance(ctx.expr(), yaplParser.Arithmetic2Context) or isinstance(
                ctx.expr(), yaplParser.Arithmetic1Context
            ):
                # print("Detnro")
                ...
                for assignment in assignments:
                    ...
                    if assignment[3].isdigit() == False and assignment[2][0] != "t":
                        if self.tabla.is_in_table(assignment[3], relative_scope):
                            informacion_fila = self.tabla.fila(
                                assignment[3], relative_scope
                            )
                            assignment[3] = f"fp[{informacion_fila.offset}]"
                    print(
                        f"    {assignment[0]} = {assignment[1]} {assignment[2]} {assignment[3]}"
                    )
        ...
        if hasattr(ctx.expr(), "INT"):
            ...
            if ctx.formal().TYPE().getText() != tipo_expr_14_15_16:
                self.errores.append(
                    f">> Intento de asignaciÃ³n incorrecto: {self.visit(ctx.expr())} a {ctx.formal().TYPE().getText()}"
                )
        ...
        #     if ctx.formal().TYPE().getText() != tipo_expr_14_15_16:
        #         self.errores.append(f'>> Intento de asignaciÃ³n incorrecto: {self.visit(ctx.expr())} a {ctx.formal().TYPE().getText()}')
    ...
    def visitWhile(self, ctx: yaplParser.WhileContext):
        ...
        condition_expression = ctx.expr(0).getText()
        if condition_expression != "true":
            postfix_condition = infix_to_postfix(condition_expression)
            tac_condition = process_postfix(postfix_condition)

            for tac in tac_condition:
                print(f"    {tac}")

            if tac_condition:
                comparison_lhs = tac_condition[-1].split()[0]
                if not comparison_lhs.startswith("t"):
                    comparison_lhs = tac_condition[-1]
                print(f"    IF {comparison_lhs} GOTO {end_label}")
            else:
                print("Error: empty!")
    ...
    def visitIf(self, ctx: yaplParser.IfContext):
        ...
        if isinstance(ctx.expr(0), yaplParser.ComparissonContext):
            ...
            print(f"        IF t0 GO TO THEN")
            print(f"        ELSE:")

            print(f"        THEN:")

            if isinstance(ctx.expr(1), yaplParser.IfContext):
                pass
    ...
    def visitDispatchExplicitA(self, ctx: yaplParser.DispatchExplicitAContext):
        ...
        propiedades = {"objeto": None, "funcion": None, "parametros": None}
